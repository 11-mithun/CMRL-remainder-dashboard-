...
function setupRowEventListeners(row) {
    ...
    // Add month dropdown change listener for auto-save

    const monthsSelect = row.querySelector('.months-select');

    if (monthsSelect) {

        monthsSelect.addEventListener('change', function() {

            // Auto-save on month change
    ...
    // Add generic change listeners for all inputs/selects to auto-save

    const inputs = row.querySelectorAll('input, select');

    inputs.forEach(input => {

        if (!input.hasAttribute('data-auto-save-listener')) {

            input.setAttribute('data-auto-save-listener', 'true');

            // Skip fields already handled above

            if (input.classList.contains('start-date-input') || 

                input.classList.contains('end-date-input') ||

                input.classList.contains('frequency-select') ||

                input.classList.contains('months-select') ||

                input.classList.contains('status-select') ||

                input.classList.contains('remarks-input')) {
            ...
            // For text inputs, also save on input (with debounce)

            if (input.tagName === 'INPUT' && input.type === 'text') {

                input.addEventListener('input', function() {

                    clearTimeout(row._autoSaveTimeout);

                    row._autoSaveTimeout = setTimeout(() => {
    ...
    if (statusSelect) {
        ...
        statusSelect.addEventListener('change', function() {
            ...
            const selection = getCurrentMonthYearSelection();

            if (!selection) return;

            saveMonthlyRowData(selection.year, selection.month, rowIndex);
    ...
    if (remarksInput) {
        ...
        remarksInput.addEventListener('input', function() {
            ...
            clearTimeout(row._remarksSaveTimeout);

            row._remarksSaveTimeout = setTimeout(() => {
...
let billTrackerMonthYearSuppressHandlers = false;

let billTrackerLastLoadedKey = null;

let billTrackerLoadAbortController = null;



function getCurrentMonthYearSelection() {
    ...
    const month = globalMonthsSelect ? (globalMonthsSelect.value || '').trim() : '';

    const yearStr = filterYearSelect ? (filterYearSelect.value || '').trim() : '';
...
function resetMonthlyFieldsForAllRows() {
...
function applyMonthlyDataToRows(records) {
    ...
    (records || []).forEach(r => {
        ...
        if (r && typeof r.rowIndex === 'number') byIndex.set(r.rowIndex, r);
    ...
    rows.forEach((row, idx) => {
        ...
        if (statusSelect) statusSelect.value = rec ? (rec.status || '') : '';

        const remarksInput = row.querySelector('.remarks-input');

        if (remarksInput) remarksInput.value = rec ? (rec.remarks || '') : '';
...
async function loadMonthlyDataForSelection(year, month) {
    ...
    if (billTrackerLastLoadedKey === key) return;



    if (billTrackerLoadAbortController) {

        billTrackerLoadAbortController.abort();

    }

    billTrackerLoadAbortController = new AbortController();



    try {

        const url = `/api/bill-tracker/load?year=${encodeURIComponent(year)}&month=${encodeURIComponent(month)}`;

        const res = await fetch(url, { signal: billTrackerLoadAbortController.signal });
        ...
        if (Array.isArray(data) && data.length > 0) {
            ...
            applyMonthlyDataToRows(data);

        } else {

            resetMonthlyFieldsForAllRows();

        }



        billTrackerLastLoadedKey = key;

    } catch (err) {

        if (err && err.name === 'AbortError') return;

        console.error('Error loading monthly bill tracker data:', err);

        if (typeof showNotification === 'function') {

            showNotification('Error loading month data. Please try again.', 'error');
...
async function saveMonthlyRowData(year, month, rowIndex) {
    ...
    try {
        ...
        const res = await fetch('/api/bill-tracker/save', {
        ...
        console.error('Error saving monthly bill tracker row:', err);
            ...
            row.innerHTML = `
                ...
                <td>
                    ...
                    <select class="status-select">
                        ...
                        <option value="Initiated" ${rowData.status === 'Initiated' ? 'selected' : ''}>Initiated</option>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (selection) {
        ...
        loadMonthlyDataForSelection(selection.year, selection.month);
...
function setupEventListeners() {
    ...
    document.getElementById('exportBtn').addEventListener('click', openBillTrackerExportDialog);
...
const BILL_TRACKER_EXPORT_MONTHS = [
...
function openBillTrackerExportDialog() {

    const dialog = document.getElementById('billTrackerExportDialog');

    if (!dialog) {

        // Fallback: export current month

        const selection = getCurrentMonthYearSelection();

        if (selection) {

            exportBillTrackerMonthWise(selection.year, selection.month);
    ...
    const typeSelect = document.getElementById('billTrackerExportType');

    const yearSelect = document.getElementById('billTrackerExportYear');

    const monthSelect = document.getElementById('billTrackerExportMonth');

    const confirmBtn = document.getElementById('billTrackerExportConfirmBtn');

    const cancelBtn = document.getElementById('billTrackerExportCancelBtn');



    if (!typeSelect || !yearSelect || !monthSelect || !confirmBtn || !cancelBtn) {
    ...
    if (!dialog.dataset.initialized) {
        ...
        typeSelect.addEventListener('change', () => {
            ...
            const exportType = (typeSelect.value || 'month').trim();

            monthSelect.disabled = exportType === 'year';
        ...
        confirmBtn.addEventListener('click', async (e) => {
            ...
            const year = parseInt((yearSelect.value || '').trim(), 10);
            ...
            try {
                ...
                if (exportType === 'year') {
                    ...
                    await exportBillTrackerYearWise(year);

                } else {

                    await exportBillTrackerMonthWise(year, month);
    ...
    const currentYearVal = filterYearSelect ? (filterYearSelect.value || '').trim() : '';

    const currentMonthVal = (document.getElementById('globalMonthsSelect')?.value || '').trim();
    ...
    if (filterYearSelect) {
        ...
        Array.from(filterYearSelect.options).forEach(opt => {
    ...
    BILL_TRACKER_EXPORT_MONTHS.forEach(m => {
...
function collectBillTrackerBaseRows() {
    ...
    return rows.map(row => ({
        ...
        months: row.querySelector('.months-status')?.textContent || '',
...
async function fetchMonthlyStatusMap(year, month) {

    const response = await fetch(`/api/bill-tracker/load?year=${encodeURIComponent(year)}&month=${encodeURIComponent(month)}`);
...
async function exportBillTrackerMonthWise(year, month) {

    const baseRows = collectBillTrackerBaseRows();
    ...
    const monthlyMap = await fetchMonthlyStatusMap(year, month);

    const sheetRows = [];

    sheetRows.push([

        'S.NO', 'EFILE NO', 'CONTRACTOR', 'START DATE', 'END DATE', 'DURATION',

        'HANDLE BY', 'FREQUENCY', 'MONTHS', 'STATUS', 'REMARKS'
    ...
    baseRows.forEach((r, idx) => {
        ...
        const monthly = monthlyMap.get(idx);

        const outMonths = `${month} ${year}`;

        const outStatus = monthly ? monthly.status : '';

        const outRemarks = monthly ? monthly.remarks : '';
    ...
    XLSX.writeFile(wb, `bill_tracker_${year}_${month}.xlsx`);
...
async function exportBillTrackerYearWise(year) {
    ...
    try {
        ...
        // Fetch all year data in one go - removing the 'lod logic' (sequential loops)

        const response = await fetch(`/api/bill-tracker/load-year?year=${year}`);
        ...
        // Create mapping: month -> { rowIndex -> {status, remarks} }

        const monthDataMap = {};

        allRecords.forEach(rec => {

            if (!monthDataMap[rec.month]) monthDataMap[rec.month] = {};

            monthDataMap[rec.month][rec.rowIndex] = {
        ...
        // Header Row includes MONTH column

        sheetRows.push([

            'S.NO', 'EFILE NO', 'CONTRACTOR', 'START DATE', 'END DATE', 'DURATION',

            'HANDLE BY', 'FREQUENCY', 'MONTH', 'STATUS', 'REMARKS'

        ]);



        // Consolidate all 12 months into ONE sheet (requested single sheet itself)

        BILL_TRACKER_EXPORT_MONTHS.forEach(month => {

            const monthlyMap = monthDataMap[month] || {};

            baseRows.forEach((r, idx) => {

                const monthly = monthlyMap[idx];
        ...
        // Auto-width columns to fix #### dates and ensure readability
        ...
        XLSX.writeFile(wb, `bill_tracker_${year}_consolidated.xlsx`);



    } catch (err) {

        console.error('Annual export failed:', err);
...
function addRow() {
    ...
    row.innerHTML = `
        ...
        <td>
            ...
            <div class="sno-cell">
                ...
                <input type="checkbox" class="sno-checkbox" data-row="${rowCounter}">
    ...
    // Trigger auto-save for the new row
...
function initializeGlobalMonthsDropdown() {
    ...
    globalMonthsSelect.addEventListener('change', function(e) {
        ...
        if (billTrackerMonthYearSuppressHandlers) return;
